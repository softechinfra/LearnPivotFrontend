...
const DateSelector = ({ data }) => {
  ...
  useEffect(() => {
    ...
    for (let batchIndex = 0; batchIndex < dates.length; batchIndex++) {
      for (let dateIndex = 0; dateIndex < dates[batchIndex].length; dateIndex++) {
        const date = new Date(dates[batchIndex][dateIndex]);
        if (date > today) {
          setSelectedBatches([batchIndex]);
          setStartDate(dates[batchIndex][dateIndex]);
          setAvailableDates(dates[batchIndex].slice(dateIndex));
          return;
        }
      }
    }
  ...
  useEffect(() => {
    if (selectedBatches.length > 0) {
      const batchDates = dates[selectedBatches[0]].filter(date => new Date(date) > today);
